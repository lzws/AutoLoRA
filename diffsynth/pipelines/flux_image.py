...
from ..vram_management import enable_vram_management, enable_auto_lora, enable_retrieve_lora, enable_k_lora,enable_DARE_lora, AutoLoRALinear, AutoWrappedModule, AutoWrappedLinear
import time
...
class FluxImagePipeline(BasePipeline):
    ...
    def enable_retrieve_lora(self):
        enable_retrieve_lora(
            self.dit,
            module_map={
                RMSNorm: AutoWrappedModule,
                torch.nn.Linear: AutoLoRALinear,
    ...
    def add_lora_emb(self, prompt_emb_posi, lora_emb):
        prompt_emb_posi['prompt_emb'] = torch.concat([prompt_emb_posi['prompt_emb'], lora_emb], dim=1)
        extra_text_ids = torch.zeros((lora_emb.shape[0], lora_emb.shape[1], 3), device=lora_emb.device, dtype=lora_emb.dtype)
        prompt_emb_posi['text_ids'] = torch.concat([prompt_emb_posi['text_ids'], extra_text_ids], dim=1)
    ...
    def __call__(
        ...
        if lora_emb is not None:
            prompt_emb_posi = self.add_lora_emb(prompt_emb_posi, lora_emb)
        # Extra input
        ...
        for progress_id, timestep in enumerate(self.scheduler.timesteps):
            ...
            # print(f"Progress: {progress_id} timestep: {timestep}")
    ...
    def get_lora_rank(
        ...
        lora_patcher=None,
        # auto retrieve
        target_layer_name='blocks.0.norm1_b.linear',
        target_step = 1
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if progress_id+1 == target_step[-1]:
                break
