...
class LoRAFromCivitai:
    ...
    def convert_state_name(self, state_dict, lora_prefix="lora_unet_", torch_dtype=torch.bfloat16, device="cpu"):
        for key in state_dict:
            if ".lora_up" in key:
                return self.convert_state_name_up_down(state_dict=state_dict, lora_prefix=lora_prefix, torch_dtype=torch_dtype, device=device)
        return self.convert_state_name_AB(state_dict=state_dict, lora_prefix=lora_prefix, torch_dtype=torch_dtype, device=device)
    ...
    def convert_state_name_up_down(self, state_dict, lora_prefix="lora_unet_", torch_dtype=torch.bfloat16, device="cpu"):
        renamed_lora_prefix = self.renamed_lora_prefix.get(lora_prefix, "")
        state_dict_ = {}
        # get alpha
        import math
        ...
        for key in state_dict:
            if ".alpha" in key:
                lora_name = key.replace('.alpha', '.lora_down.weight')
                lora_alpha = state_dict[key].item() / state_dict[lora_name].shape[0]
        ...
        for key in state_dict:
            ...
            if not key.startswith(lora_prefix):
                continue
            weight_up = state_dict[key]
            weight_down = state_dict[key.replace(".lora_up", ".lora_down")]
            if len(weight_up.shape) == 4:
            ...
            state_dict_[target_name.replace(".weight",".lora_B.weight")] = (weight_up * lora_alpha).to(device=device, dtype=torch_dtype)
            state_dict_[target_name.replace(".weight",".lora_A.weight")] = (weight_down * lora_alpha).to(device=device, dtype=torch_dtype)
        return state_dict_


    def convert_state_dict_AB(self, state_dict, lora_prefix="", alpha=1.0, device="cuda", torch_dtype=torch.float16):
        state_dict_ = {}
        # print(f"convert_state_dict_AB lora_prefix: {lora_prefix}")
        for key in state_dict:
    ...
    def convert_state_name_AB(self, state_dict, lora_prefix="", device="cpu", torch_dtype=torch.bfloat16):
        state_dict_ = {}
        # print(f"convert_state_dict_AB lora_prefix: {lora_prefix}")
        # get alpha
        import math
        ...
        for key in state_dict:
            if ".alpha" in key:
                lora_name = key.replace('.alpha', '.lora_A.weight')
                lora_alpha = state_dict[key].item() / state_dict[lora_name].shape[0]
                lora_alpha = math.sqrt(lora_alpha)
                break
        ...
        for key in state_dict:
            ...
            # print(f"key: {key}")
            weight_up = state_dict[key]
            weight_down = state_dict[key.replace(".lora_B.", ".lora_A.")]
            if len(weight_up.shape) == 4:
                weight_up = weight_up.squeeze(3).squeeze(2)
                weight_down = weight_down.squeeze(3).squeeze(2)
                # lora_weight = alpha * torch.mm(weight_up, weight_down).unsqueeze(2).unsqueeze(3)
    ...
    def get_converted_lora_state_dict(self, model, state_dict_lora, torch_dtype=torch.bfloat16, device="cpu"):
        ...
        for lora_prefix, model_class in zip(self.lora_prefix, self.supported_model_classes):
            ...
            for model_resource in ["diffusers","civitai"]:
                try:
                    state_dict_lora_ = self.convert_state_name(state_dict=state_dict_lora, lora_prefix=lora_prefix, device=device, torch_dtype=torch_dtype)
                    ...
                    for name in state_dict_lora_:
                        if name.replace('.lora_B','').replace('.lora_A','') not in state_dict_model and name.replace('.lora_A','.module').replace('.lora_B','.module') not in state_dict_model:
    ...
    def get_converted_lora_state_dict_new(self, model=FluxDiT, state_dict_lora={}, flux_dit_keys=[], torch_dtype=torch.bfloat16, device="cpu"):

        for lora_prefix, model_class in zip(self.lora_prefix, self.supported_model_classes):
            if model != model_class:
                continue
            
            state_dict_model = flux_dit_keys
            for model_resource in ["diffusers","civitai"]:
                try:
                    state_dict_lora_ = self.convert_state_name(state_dict=state_dict_lora, lora_prefix=lora_prefix, device=device, torch_dtype=torch_dtype)
                    print(f"state_dict_lora_1:{list(state_dict_lora_.keys())[:10]}")

                    converter_fn = model.state_dict_converter().from_diffusers if model_resource == 'diffusers' \
                        else model.state_dict_converter().from_civitai
                    state_dict_lora_ = converter_fn(state_dict_lora_)

                    if isinstance(state_dict_lora_, tuple):
                        state_dict_lora_ = state_dict_lora_[0]

                    print(f"state_dict_lora_2:{list(state_dict_lora_.keys())[:10]}")
...
class FluxLoRAConverter:
    ...
    def convert_state_name(self, state_dict, lora_prefix="lora_unet_"):
        for key in state_dict:
            if ".lora_up" in key:
                return self.convert_state_name_up_down(state_dict=state_dict, lora_prefix=lora_prefix)
        return self.convert_state_name_AB(state_dict=state_dict, lora_prefix=lora_prefix)

    def convert_state_name_AB(self, state_dict, lora_prefix=""):
        state_dict_ = {}
        ...
        for key in state_dict:
            ...
            state_dict_[target_name.replace(".weight",".lora_B.weight")] = (weight_up * lora_alpha)
            state_dict_[target_name.replace(".weight",".lora_A.weight")] = (weight_down * lora_alpha)
        return state_dict_

    def convert_state_name_up_down(self, state_dict, lora_prefix="lora_unet_"):
        ...
        renamed_lora_prefix = {}
        renamed_lora_prefix = renamed_lora_prefix.get(lora_prefix, "")
    ...
    def align_to_all_format(self,state_dict):
        for lora_prefix, model_class in zip(["lora_unet_", "transformer."], [FluxDiT, FluxDiT]):
            state_dict_lora_convert_name = self.convert_state_name(state_dict=state_dict, lora_prefix=lora_prefix)
            # print(f"state_dict_lora_1:{list(state_dict_lora_convert_name.keys())[:10]}")
            for model_resource in ["diffusers","civitai"]:
                try:
                    converter_fn = FluxDiT.state_dict_converter().from_diffusers if model_resource == 'diffusers' \
                        else FluxDiT.state_dict_converter().from_civitai
                    state_dict_lora_ = converter_fn(state_dict_lora_convert_name)
                    if isinstance(state_dict_lora_, tuple):
                        state_dict_lora_ = state_dict_lora_[0]
                    # print(f"state_dict_lora_2:{list(state_dict_lora_.keys())[:10]}")
                    if len(state_dict_lora_) == 0:
                    ...
                    print(f"can not convert lora state dict error   {str(e)}")
        return {}
